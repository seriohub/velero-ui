...
//const xLabels = new Array(60).fill(0).map((_, i) => `${i}`);
const xLabels = new Array(60).fill(0).map((_, i) => (i % 5 === 0 ? `${i}` : ''));
const yLabels = new Array(24).fill(0).map((_, i) => `${i}`);

export function DayHeatmap({ data, heatmapScheduleName }: DayHeatmapProps) {
    ...
    <>
      ...
      >
        <List
          ...
          size="sm"
          center
          icon={<IconClock style={{ width: rem(16), height: rem(16) }} color="green" />}
        >
          {renderScheduleName}
        </List>
      </Box>
