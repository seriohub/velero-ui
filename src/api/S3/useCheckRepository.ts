...
export const useCheckRepository = () => {
    ...
    const checkRepository = async (repositoryURL: string, backupStorageLocation: string, repositoryName: string, repositoryType: string, volumeNamespace: string) => {
        try {
    ...
    return { checkRepository, data, fetching, error };
};
