...
export function PVBSummary({ backupName }: PVBSummaryProps) {
  ...
  return (
    <Card shadow="sm" padding="lg" radius="md" withBorder h="100%">
      <Card.Section withBorder inheritPadding p="xs">
        <Text fw={600}>Pod volumes [{data?.payload.length}]</Text>
      </Card.Section>
      <Card.Section p="sm">
        {!data?.payload || (data.payload.length === 0 && <Text>No volumes</Text>)}
        {data?.payload && data.payload.length > 0 && <ScrollArea h="600px">{items}</ScrollArea>}
      </Card.Section>
    </Card>
  );
}
